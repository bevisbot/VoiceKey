...
/// 高频词表:用户维护的常用词/专有名词,润色纠错时同音/近音优先向这些词靠拢。
/// 文件:~/Library/Application Support/VoiceKey/hotwords.txt(一行一个词,# 开头为注释)
enum HotWords {
    static var fileURL: URL {
        let dir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("VoiceKey", isDirectory: true)
    ...
    static func ensureTemplate() {
        guard !FileManager.default.fileExists(atPath: fileURL.path) else { return }
        let tmpl = """
        # 高频词表:一行一个词,保存即生效(无需重启)。
        # 语音转写出现同音/近音误识别时,润色会优先纠正为这里的写法。
        # 适合放:人名、产品名、公司名、行业术语、常用英文词等。示例(可删):
        # 在途宝
        # VoiceKey
        # staging
...
enum CloudPolisher {
    ...
    - 保持原文语种,绝不翻译:纯英文原文就输出英文,纯中文就输出中文,中英混排各自保留原文。不要把英文翻成中文、也不要把中文翻成英文。
    - 拿不准的专有名词保持原样。
    ...
    static func polish(_ raw: String) async -> String {
        ...
        // 高频词表:同音/近音误识别优先向这些词靠拢(词表为空则不注入)
        let hotwords = HotWords.load()
        let hotwordDirective = hotwords.isEmpty ? "" : """

        说话人的高频词表(原文中出现与这些词同音/近音的字词时,优先纠正为词表里的写法,并严格保持词表中的大小写和写法、不要给词加引号;与原文读音明显不符的不要硬套):
        \(hotwords.joined(separator: "、"))
        """

        // 把原文用分隔符包成纯数据,降低被当成指令/问题来"回答"的概率
        let userMsg = """
        \(langDirective)\(hotwordDirective)
