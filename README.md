...
在**任意 App 的任意输入框**,按一下快捷键说话,实时转写 → 自动插入到光标处。
专为**中文 / 中英混杂**优化,延迟极低(松手到出字约 0.3 秒)。
...
| 环节 | 用什么 | 说明 |
|---|---|---|
| 转写 | 火山引擎 **豆包流式语音识别2.0**(Seed) | WebSocket 双向流式 `bigmodel_async`,边说边传,松手即得(~0.3s);自带标点/ITN,直接输出 |
...
**没有降级**:断网 / 未配置凭据 / 调用失败 → 直接提示失败(不回退本地)。已移除 Whisper、Apple SpeechTranscriber、Foundation Models、自定义词表、熔断、qwen 润色等(2026-06 移除润色:LLM 二次润色偶尔改变原意,纯豆包输出更忠实)。
...
菜单栏有「填写火山 API 凭据」入口。火山凭据在语音技术控制台 → 应用管理获取,并需「开通正式版」(按时长后付费,先扣已购资源包)。

## 目录结构

```
Sources/VoiceKey/
├ main.swift / AppDelegate.swift   入口 + 编排(快捷键/录音/流式/粘贴/菜单)
...
> 不入库(`.gitignore`):`.build/`、`*.app`。运行时 `volcano.txt` 在 App 支持目录,不在仓库内。
...
- 总延迟:约 0.3~0.5s(即转写延迟,无二次处理)

## 诊断

`/tmp/voicekey-timing.log` 记录每次「转写[火山]」耗时、原文及失败原因;HotKey 事件监听异常也写这里。

## 可调项

- 改快捷键 / 悬浮条样式:见对应源文件
- 如需恢复 LLM 润色环节:见 git 历史(`AliyunCloud.swift`,2026-06 移除)
