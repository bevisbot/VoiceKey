...
专为**中文 / 中英混杂**优化,延迟低(松手到出字约 1.5~3 秒)。

## 架构(单一链路,无降级)

```
按一下右 Command → 边录边推流火山(实时识别)→ 再按一下结束
   → 松手 ~0.3s 出转写 → qwen-plus 润色(~1~2.5s,带高频词表)→ 自动 ⌘V 粘贴
```

| 环节 | 用什么 | 说明 |
|---|---|---|
| 转写 | 火山引擎 **豆包流式语音识别2.0**(Seed) | WebSocket 双向流式 `bigmodel_async`,边说边传,松手即得(~0.3s);自带标点/ITN |
| 润色 | 阿里云 **qwen-plus** | 去口头语、补标点、按上下文纠正同音字/词边界。三重约束:**只校对不回答**(问句/指令不会被作答)、**语种锁定**(代码判主语种注入硬指令,英文绝不被翻成中文)、**高频词表**(同音/近音优先纠正为用户词表里的写法)|
...
**没有降级**:断网 / 未配置凭据 / 调用失败 → 直接提示失败(不回退本地);润色失败原样返回转写文本。已移除 Whisper、Apple SpeechTranscriber、Foundation Models、熔断等。
...
| `hotwords.txt` | **高频词表**,一行一个词(人名/产品名/术语/常用英文词)。转写同音/近音误识别时润色优先纠正为词表写法。保存即生效 |

菜单栏有「填写火山 API 凭据」「填写阿里云 Key」「编辑高频词表」三个入口。火山凭据在语音技术控制台 → 应用管理获取,并需「开通正式版」(按时长后付费,先扣已购资源包)。
...
├ AliyunCloud.swift                qwen-plus 润色(CloudPolisher)+ 凭据(AliyunConfig)+ 高频词表(HotWords)
├ NetworkMonitor.swift             网络可达性
├ RecorderHUD.swift                底部悬浮控件(实时波形 + 状态)
└ TextInserter.swift               剪贴板 + ⌘V 插入
```

> 不入库(`.gitignore`):`.build/`、`*.app`。运行时 `volcano.txt` / `aliyun.txt` / `hotwords.txt` 在 App 支持目录,不在仓库内。
...
2. **麦克风**:首次录音弹窗允许。
3. 填好 `volcano.txt` + `aliyun.txt` 两套凭据;高频词表 `hotwords.txt` 按需维护。

## 性能(实测)

- 火山转写:松手后 ~0.25~0.45s(边录边传,几乎瞬时)
- qwen-plus 润色:~1.0~2.5s
- 总延迟:约 1.5~3s

## 诊断

`/tmp/voicekey-timing.log` 记录每次「转写[火山](含原文)/ 润色[qwen-plus]」耗时及失败原因;HotKey 事件监听异常也写这里。
